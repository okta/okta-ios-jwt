...
open class OktaJWTValidation: NSObject {

    /**
         Validates the issuer against the well-known endpoint or given issuer.
         - parameters:
             - dirtyIssuer: Issuer claim from the JWT
             - validIssuer: Issuer to call the well-known endpoint against for validation
         - returns:
    ...
    open class func hasValidIssuer(_ dirtyIssuer: String?, validIssuer: String?) -> Bool {
        ...
        if let wellKnownIssuer = OktaAPI.getIssuerEndpoint(issuer: validIssuer!) {
    ...
    /**
         ...
         - parameters:
             ...
             - validAudience: Audience to perform the validation against
         - returns:
    ...
    open class func isIssuedInFuture(_ iat: Date?, leeway: Int?) -> Bool {
        ...
        if let givenLeeway = leeway {
            // Use given leeway
            print(now.addingTimeInterval(Double(givenLeeway)))
    ...
    /**
         ...
         - parameters:
             ...
             - validNonce: Nonce to perform the validation against
         - returns:
    ...
    /**
         ...
         - parameters:
             ...
             - validClaim: Given claim to perform the validation against
         - returns:
